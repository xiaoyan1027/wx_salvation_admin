<?php
    ...
    /**
     * 分解当前时间和cron字符串 得到 上次和下次运行时间，当上次和下次运行时间相同时运行本次cron
     *
     * @author:Riven
     * @date:2011-09-22
     */
    class Lib_cron_tdcron {
       ...
       /**
         ...
         * @return  bool
         */
        static public function checkExecCase($expression, $timestamp = null)
        {
            $key_arr  = array(IDX_MINUTE, IDX_HOUR, IDX_DAY, IDX_MONTH, IDX_WEEKDAY);

                $now_time_arr = self::getTimestamp($timestamp);

                // Calculate date/time

                $expression_time_arr  = self::getExpression($expression, false);
                foreach($key_arr as $v)
                {
                    if( ! in_array($now_time_arr[$v], $expression_time_arr[$v]))
                    {
                        return false;
                    }
                }
         ...
         static private function getExpression($expression, $reverse=false) {
            ...
            if (!isset(self::$pcron[$expression])) {
                ...
                try {
                    ...
                    self::$pcron[$expression]       = lib_cron_cronentry::parse($expression);
